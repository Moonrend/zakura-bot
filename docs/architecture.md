...
The selectable Markdown subset uses a line-based fence parser. A closing fence must use the opening character, be at least as long, and occupy its own line; code strings and shorter fences remain verbatim. Unterminated code blocks stay formatted as chunks arrive. Backtick and tilde fences support up to three spaces of indentation, and Markdown line endings normalize CRLF/CR. Inline code matches whole backtick runs of equal length, preserving embedded runs and link syntax as code. Matching runs are indexed once per line so unmatched runs do not repeatedly scan long streams. On the arriving final line, unfinished inline-code spans remain literal until the delimiter closes; their links cannot activate mid-stream. Inline HTTP(S) links keep balanced and escaped URL parentheses; incomplete destinations remain literal. Raw replies bypass Markdown parsing, and HTML remains text.
