...
export default function GroupsScreen() {
  ...
  return <ScrollView className="flex-1 bg-app" keyboardShouldPersistTaps="handled" contentContainerStyle={{ padding: 20, paddingTop: Math.max(insets.top, 20), paddingBottom: Math.max(insets.bottom, 20), maxWidth: 760, width: "100%", alignSelf: "center" }}>
    <View className="mb-5 flex-row items-center justify-between"><Text accessibilityRole="header" className="text-[20px] font-semibold text-ink">Groups</Text>
    ...
    <View className="mb-6 flex-row items-end gap-3">
      <Field accessibilityLabel="New group name" value={name} onChangeText={setName} placeholder="Group name" maxLength={64}
      ...
      <Pressable disabled={disabled || !name.trim()} accessibilityRole="button" accessibilityLabel="Create group" onPress={() => void change({ type: "create", id: uid("group"), name })}
        className="mb-px min-h-11 justify-center rounded-xl bg-accent px-4"><Text className="font-semibold text-app">Create</Text></Pressable>
    </View>
    {groups.sections.map((section, index) => <View key={section.id} className="mb-4 gap-2" testID={`group-${section.id}`}>
      {renaming === section.id ? <View className="flex-row items-end gap-3"><Field accessibilityLabel="Rename group" value={renamed} onChangeText={setRenamed} maxLength={64} containerClassName="min-w-0 flex-1" />
    ...
    <Text accessibilityRole="header" className="mb-3 mt-5 text-[16px] font-semibold text-ink">Bots</Text>
    {agents.map((agent) => {
      const key = botGroupKey(agent), sectionId = groups.assignments[key];
      return <View key={agent.id} className="mb-3">
        ...
        {moving === key ? <View className="mt-3 gap-2">{[{ id: "", name: "Ungrouped" }, ...groups.sections].map((section) => <Pressable key={section.id} disabled={disabled} accessibilityRole="button" accessibilityLabel={`Place ${agent.name} in ${section.name}`}
          onPress={() => void change({ type: "assign", botKey: key, sectionId: section.id || null })} className="min-h-11 justify-center px-3"><Text className="text-ink">{section.name}</Text></Pressable>)}</View> : null}
      </View>;
    })}
