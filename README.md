...
Streaming Markdown keeps backticks inside code strings and supports matching backtick or tilde fences, including longer fences around Markdown examples. Inline code also requires matching backtick runs, keeping embedded backticks and link syntax literal. An unfinished inline-code span on the arriving line stays literal until its closing delimiter arrives. Links preserve balanced or escaped parentheses in their destination. Reaching the bottom by scrolling also restores transcript focus when a focused Jump to latest control disappears.
