...
Sending from the composer brings the transcript back to the latest messages; incoming replies preserve your position while reading history. Jump to latest and disappearing Retry, suggestion and error controls keep keyboard focus in the transcript. Sending a suggestion preserves the draft. History timestamp corrections cannot duplicate bubbles through a collision with date separators. Long system notices wrap within narrow conversations. Raw and Markdown streams show a cursor beside the arriving text; unmatched Markdown delimiters remain visible. Unsupported file drops cannot navigate away from any app route. Mixed image/text paste keeps the text and explains that the image was not uploaded.

Roster removal and changing search matches keep keyboard navigation in the agent list or the replacement conversation without moving focus away from another draft. File, image and card-action links have a full 44px minimum hit area; primary and danger actions retain their supplied styles.

Streaming Markdown keeps backticks inside code strings and supports matching backtick or tilde fences, including longer fences around Markdown examples. Links preserve balanced or escaped parentheses in their destination. Reaching the bottom by scrolling also restores transcript focus when a focused Jump to latest control disappears.
