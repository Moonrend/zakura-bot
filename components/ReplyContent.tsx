...
export function ReplyContent({ message }: { message: ChatMessage }) {
  ...
  return (
    <View className="min-w-0 max-w-full gap-2">
      {message.replyTo ? <Text className="mb-1 border-l-2 border-accent-border pl-2 text-[11px] text-ink-secondary">Reply</Text> : null}
          ...
          {card.table ? <ScrollView horizontal className="max-w-full" accessibilityLabel="Card table">
            <View>
              <View className="flex-row border-b border-hairline">
                {card.table.headers.map((header, index) => <Text key={index} className="w-36 p-2 text-[12px] font-semibold text-ink">{header}</Text>)}
              </View>
              {card.table.rows.map((row, index) => <View key={index} className="flex-row">
                {row.map((cell, cellIndex) => <Text testID="message-text" key={cellIndex} className="w-36 p-2 text-[12px] text-ink" selectable>{cell}</Text>)}
              </View>)}
            </View>
          </ScrollView> : null}
        </View>
